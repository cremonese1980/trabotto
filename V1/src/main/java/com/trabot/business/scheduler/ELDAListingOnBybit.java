...
@Slf4j
@Component
public class ELDAListingOnBybit extends ListingNewCryptoScheduler {

    private static final int X2_INDEX = 0, X3_INDEX = 1, X4_INDEX = 2, X5_INDEX = 3;

    private static final String PAIR = "ELDAUSDT";// ELDAUSDT
    private static final BigDecimal BASE_PRICE = new BigDecimal(0.015);// 0.015
    private static final int[] PRICE_MULTIPLIERS = {1, 2, 3, 4, 5, 6}; // {1, 2, 3, 4, 5, 6};
    private static final String LISTING_CRON = "0 0 15 30 5 ?";// "0 0 15 30 5 ?"
    private static final String LISTING_CRON_ONE_SECOND_BEFORE = "59 59 14 30 5 ?";// "59 59 14 30 5 ?"
    ...
    private Map<Integer, Map<Integer, Map<TradeType, Position>>> positionMapByUsdtByPriceMultiplierByTradeType;

    public ELDAListingOnBybit(BybitManager bybitManager) throws ParseException {
	super(PAIR, BASE_PRICE, LISTING_CRON, PRICE_SCALE, PRICE_MULTIPLIERS, bybitManager, false);
	...
	positionMapByUsdtByPriceMultiplierByTradeType = new HashMap<Integer, Map<Integer, Map<TradeType, Position>>>();
	BigDecimal price = null;
	for (int size : SIZE_LIST) {

	    positionMapByUsdtByPriceMultiplierByTradeType.put(size, new HashMap<Integer, Map<TradeType, Position>>());
    ...
    private Position getPosition(int size, int multiplier, TradeType tradeType) {
    ...
    public void scheduledPlaceOrder_1_ter() {

	waitTargetTimestamp(listingDate.getTime() - 200);
	Position position = getPosition(HUNDRED_USDT, 2, TradeType.LIMIT);
	log.info("1st method ter - {}", position);
	placeOrder(position);
    }
    @Scheduled(cron = LISTING_CRON_ONE_SECOND_BEFORE)
    public void scheduledPlaceOrder_1_bis() {

	waitTargetTimestamp(listingDate.getTime() - 100);
	Position position = getPosition(HUNDRED_USDT, 2, TradeType.LIMIT);
	log.info("1st method bis - {}", position);
	placeOrder(position);
    }
    @Scheduled(cron = LISTING_CRON_ONE_SECOND_BEFORE)
	...
	Position position = getPosition(TEN_USDT, 2, TradeType.IOC);
	...
	waitTargetTimestamp(listingDate.getTime() - 10);
	Position position = getPosition(TEN_USDT, 2, TradeType.LIMIT);
	...
	Position position = getPosition(HUNDRED_USDT, 2, TradeType.IOC);
	...
	waitTargetTimestamp(listingDate.getTime());
	Position position = getPosition(FIVEHUNDREDS_USDT, 2, TradeType.IOC);
	...
	waitTargetTimestamp(listingDate.getTime());
	Position position = getPosition(FIVEHUNDREDS_USDT, 2, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime());
	Position position = getPosition(TEN_USDT, 3, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime());
	Position position = getPosition(HUNDRED_USDT, 3, TradeType.IOC);
	...
	Position position = getPosition(HUNDRED_USDT, 3, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime() + 5);
	Position position = getPosition(FIVEHUNDREDS_USDT, 3, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime());
	Position position = getPosition(FIVEHUNDREDS_USDT, 4, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime() + 10);
	Position position = getPosition(HUNDRED_USDT, 4, TradeType.LIMIT);
	...
	Position position = getPosition(HUNDRED_USDT, 5, TradeType.LIMIT);
	...
	waitTargetTimestamp(listingDate.getTime() + 800);
	Position position = getPosition(HUNDRED_USDT, 6, TradeType.LIMIT);
	log.info("21th method - {}", position);
	placeOrder(position);
    }
    @Scheduled(cron = LISTING_CRON)
    public void scheduledPlaceOrder_21_bis() {

	waitTargetTimestamp(listingDate.getTime() + 900);
	Position position = getPosition(HUNDRED_USDT, 6, TradeType.LIMIT);
	log.info("21th bis method - {}", position);
	placeOrder(position);
    }
